...
final class GraphImpl implements Graph {
    ...
    private final RDF<Graph, Triple, RDFTerm, BlankNodeOrIRI, IRI, BlankNode, Literal> rdf;
    ...
    public void add(BlankNodeOrIRI subject, IRI predicate, RDFTerm object) {
        ...
        Triple result = rdf.createTriple(newSubject, newPredicate, newObject);
        triples.add(result);
    }
    ...
    public void add(Triple triple) {
        ...
        if (newSubject == triple.getSubject()
            ...
            Triple result = rdf.createTriple(newSubject, newPredicate,
                    newObject);
            triples.add(result);
        }
    }
    ...
    private <T extends RDFTerm> RDFTerm internallyMap(T object) {
        if (object instanceof BlankNode && !(object instanceof BlankNodeImpl)) {
            ...
            if (literal.getLanguageTag().isPresent()) {
                return rdf.createLiteral(literal.getLexicalForm(), literal
                        .getLanguageTag().get());
            } else {
                return rdf.createLiteral(literal.getLexicalForm(),
